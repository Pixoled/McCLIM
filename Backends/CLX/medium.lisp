...
(defun line-style-effective-dashes (line-style medium)
  (let ((scale  (line-style-scale line-style medium))
    ...
    (flet ((scale-dash (dash)
             (* dash scale)))
      (if (member dashes '(t nil))
          (scale-dash 3)
          (map 'list #'scale-dash dashes)))))
    ...
    (when gc
      (let ((old-line-style (medium-line-style medium)))
        ...
        (unless (and (eq (line-style-dashes line-style)
                     ...
                     (eq (line-style-unit line-style)
                         (line-style-unit old-line-style)))
          (let* ((dash-pattern (line-style-effective-dashes line-style medium))
                 (dash-pattern (if (atom dash-pattern)
                                   (round dash-pattern)
                                   (mapcar #'round dash-pattern)))
                 (dashesp (if (line-style-dashes line-style) :dash :solid)))
            (setf (xlib:gcontext-line-style gc) dashesp
                  (xlib:gcontext-dashes gc) dash-pattern)))))))
  ...
  (let ((old-tr     (medium-transformation medium))
    ...
    (unless (and (eq :coordinate (line-style-unit line-style))
      ...
      (when-let ((gc (slot-value medium 'gc)))
        (let* ((thickness    (round (line-style-effective-thickness line-style medium)))
               (dash-pattern (line-style-effective-dashes line-style medium))
               (dash-pattern (if (atom dash-pattern)
                                 (round dash-pattern)
                                 (mapcar #'round dash-pattern)))
               (dashesp      (if (line-style-dashes line-style) :dash :solid)))
          (setf (xlib:gcontext-line-style gc) dashesp
                (xlib:gcontext-dashes gc)     dash-pattern
                (xlib:gcontext-line-width gc) thickness))))
    new-tr))
...
(defmethod medium-draw-points* ((medium clx-medium) coord-seq)
  (with-transformed-positions ((sheet-native-transformation
                               ...
                               coord-seq)
    (with-clx-graphics () medium
      (let ((circumference (line-style-effective-thickness line-style medium)))
        (if (< circumference 2)
            (do-sequence ((x y) coord-seq)
              (let ((x (round-coordinate x))
                ...
                (when (and (typep x '(signed-byte 16))
                           (typep y '(signed-byte 16)))
                  (xlib:draw-point mirror gc x y))))
            (let ((radius        (round circumference 2))
    ...
    (with-transformed-position (tr left top)
      (with-transformed-position (tr right bottom)
        (let ((min-x (round-coordinate (min left right)))
          ...
          (let ((^cleanup nil))
            (unwind-protect (xlib:draw-rectangle (port-lookup-mirror (port medium)
                                                                     (medium-sheet medium))
